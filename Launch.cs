...
namespace QRCodeGenerator
{
    class Launch : Form
    {
        ...
        static void Main()
        {
            ...
            foreach (var dll in exec.GetManifestResourceNames())
            {
                var dllPath = Path.Combine(Path.GetDirectoryName(exec.Location), "zxing.dll");
                if (!File.Exists(dllPath))
                {
                    using (var ms = new MemoryStream())
                    {
                        using (var dllStream = exec.GetManifestResourceStream(dll))
                            dllStream.CopyTo(ms);
